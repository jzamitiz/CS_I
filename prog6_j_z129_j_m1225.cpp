/*
   File: prog6_j_z129_j_m1225.cpp

   Author: Josue Zamitiz-Ayala & Jose Medina
   C.S.1428.002
   Lab Section: L22 & L17
   Program: #6
   Due Date: 11/30/18
...
int main ( )
{
   const string AUTHORS = "Josue Zamitiz-Ayala & Jose Medina",
                LAB_SECTION = "L22 & L17",
                DUE_DATE = "11/30/18";
...
/*
      ...
      Receives: output file variable,
                AUTHORS, LECTURE_SECTION, DUE_DATE, (string constants)
                LAB_SECTION, (integer constant)
...
/*
      ...
      The function, printReportHeadings, writes to an output file the report
      title and the column headers. The title is centered and the column
      headears are appropriately spaced to input data in tabular form.
      Example:
 ...
 ID#     Hours    Hourly   Overtime   Gross     State   Federal    Net
      ...
      Receives: output file variable
      Constants: none
      Returns: nothing; prints title and the column headers
...
void printReportHeadings ( ofstream &fout )
{
    ...
    fout << "        Worked     Rate      Hours      Pay       Tax       Tax"
         << "       Pay" << endl;
}
...
/*
      ...
      Author(s): Josue Zamitiz-Ayala & Jose Medina
      C.S.1428.002
      Lab Section: L22 & L17
      Program 6 Function dataIn
      11/30/18

      The function, dataIn, uses a for loop to read data from an input file
      named prog6_002inp.txt. Reads employees ID#s, hours worked and their
      payrate. It stores these values into parallel arrays; the ID#s into a 1D
      array, hours worked and payrate into the corresponding 2D array of reals.

      Receives: input file variable, protected 1D array of integers ID#s,
      ...
      Returns: nothing; reads data from input file
*/
    ...
    for ( int row = 0; row < ROWS; row++ )
    {
        if ( payroll[row][HRS_WRKD] <= CUT_OFF )
            ...
            payroll[row][GROSS] = ( CUT_OFF * payroll[row][PAYRATE] )
                                + ( ( payroll[row][HRS_WRKD] - CUT_OFF )
                                * ( 1.5 * payroll[row][PAYRATE] ) );
    }
}
...
void federalTax ( double payroll[][COLS] )
{
    for ( int row = 0; row < ROWS; row++ )
    {
        if ( payroll[row][GROSS] <= TAX_CUT_OFF )
            payroll[row][FED_TAX] = payroll[row][GROSS]
                                  * LOW_TAX_RATE;
        else
...
/*
      ...
      Net pay is equal to gross minues state and federal.
...
/*
      ...
      The function, writeFileLocation, prints an appropriate message to the
      console screen indicating to the user the name of the output file to which
      the results have been written.

      Receives: 1D string array called output_filename
      Constants: declared constant integer MAX_LENGTH_FN
      Returns: nothing; prints message to console
*/
